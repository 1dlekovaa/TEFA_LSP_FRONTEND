<template>
  <div class="space-y-6">
    ...
    <Breadcrumb :items="[
      { label: 'Referensi' },
      { label: 'Data Asesi', to: '/referensi/asesi' },
      { label: 'Detail' }
    ]" />
    ...
    <div class="flex justify-between items-center">
      <div>
        <h1 class="text-3xl font-bold text-gray-900">Detail Asesi</h1>
      </div>
      <NuxtLink 
        to="/referensi/asesi"
      ...
      <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div class="md:col-span-2 space-y-4">
          <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              <label class="text-sm font-medium text-gray-600">No Peserta:</label>
              <p class="text-gray-900 font-semibold">{{ item.no_peserta }}</p>
            </div>
            <div>
              <label class="text-sm font-medium text-gray-600">Kelas:</label>
              <p class="text-gray-900 font-semibold">{{ item.kelas }}</p>
            </div>
            <div>
              <label class="text-sm font-medium text-gray-600">Tahun Aktif:</label>
              <p class="text-gray-900 font-semibold">{{ item.tahun_aktif_id }}</p>
            </div>
            <div>
          ...
          <div class="flex gap-2 mt-6 pt-4 border-t">
            <NuxtLink 
              :to="`/referensi/asesi/${item.id}/edit`"
...
const item = ref<Asesi | null>(null)
...
onMounted(async () => {
  ...
  try {
    ...
    const response = await asesiService.getById(id)
...
const handleDelete = async () => {
  ...
  try {
    const response = await asesiService.delete(item.value.id)
    if (response.success) {
      success('Data berhasil dihapus')
      router.push('/referensi/asesi')
    } else {
