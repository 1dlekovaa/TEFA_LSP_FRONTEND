...
<template>
  <aside
    class="w-64 bg-[#1C6B93] text-white p-4 space-y-2 flex flex-col h-screen"
  >
    <!-- HEADER SIDEBAR / LOGO -->
    ...
    <!-- MENU ITEMS -->
    <div
      class="flex-1 flex flex-col space-y-2 overflow-y-auto overflow-x-hidden pr-2 hide-scrollbar"
    >
      ...
      <div v-if="isAdmin">
        ...
        >
          ...
          <ChevronRight
            ...
            :class="openMenus.referensi ? 'rotate-90' : ''"
          />
        </button>

        <div
          class="overflow-hidden transition-all duration-300"
          :class="openMenus.referensi ? 'max-h-96' : 'max-h-0'"
        >
          <NuxtLink
            to="/referensi/asesor"
            class="block ml-6 py-2 rounded transition-all duration-200 hover:bg-white/20"
          >
            Data Asesor
          </NuxtLink>
          <NuxtLink
      ...
      <div v-if="isAdmin || isAsesor">
        ...
        >
          ...
          <ChevronRight
            class="w-4 transition duration-300"
            :class="openMenus.uji ? 'rotate-90' : ''"
          />
        </button>

        <div
          class="overflow-hidden transition-all duration-300"
          :class="openMenus.uji ? 'max-h-60' : 'max-h-0'"
        >
          <NuxtLink
            to="/uji/apl01"
      ...
      <div v-if="isAdmin">
        ...
        >
          ...
          <ChevronRight
            class="w-4 transition duration-300"
            :class="openMenus.pengaturan ? 'rotate-90' : ''"
          />
        </button>

        <div
          class="overflow-hidden transition-all duration-300"
          :class="openMenus.pengaturan ? 'max-h-40' : 'max-h-0'"
        >
          <NuxtLink
            to="/pengaturan/user"
            class="block ml-6 py-2 hover:bg-white/20 rounded"
    ...
    <div class="mt-auto pt-4 border-t border-white/20">
      <button
        @click="handleLogout"
        class="w-full px-4 py-2 bg-red-500 hover:bg-red-600 rounded text-white transition"
      >
        Logout
      </button>
